...
struct t8dg_local_values
{
  ...
  t8dg_quad_values_t *element_transformed_gradient_tangential_vectors[3];    /**< For each element, (d_x F^-1)(tau_d), needed to calculate the gradient */
  ...
  t8dg_dof_values_t  *face_normal_vectors[MAX_FACES][3];                  /**< For each face and nodal basis point the 3-dim normal vector in image space*/
};

typedef struct t8dg_local_values_fn_evaluation_data
{
  t8dg_geometry_t    *geometry;
...
/*
static double      *
t8dg_local_values_get_face_trafo_quad_weights (const t8dg_local_values_t * values, const t8_locidx_t idata,
                                                           const int faceindex)
{
...
static double      *
t8dg_local_values_get_element_quad_trafo_weights (const t8dg_local_values_t * values, const t8_locidx_t idata)
{
...
double             *
t8dg_local_values_get_face_normal_vector (const t8dg_local_values_t * values, const t8_locidx_t idata,
                                       ...
                                       t8dg_local_values_t * dest_values, t8_locidx_t dest_idata)
{
  ...
  for (idim = 0; idim < DIM3; idim++) {
    t8dg_quad_values_copy_from_index_to_index (src_values->element_transformed_gradient_tangential_vectors[idim], src_idata,
                                               dest_values->element_transformed_gradient_tangential_vectors[idim], dest_idata);
  ...
  for (iface = 0; iface < src_values->max_num_faces; iface++) {
    ...
    for (idim = 0; idim < DIM3; idim++) {
      ...
      t8dg_dof_values_copy_from_index_to_index (src_values->face_normal_vectors[iface][idim], src_idata,
                                                dest_values->face_normal_vectors[iface][idim], dest_idata);
    }
  }
}
...
#if 0
 /*TODO*/ void
t8dg_local_values_set_element (t8dg_local_values_t * local_values, t8_locidx_t idata)
{
  double              sqrt_gram_det, face_sqrt_gram_det;
  int                 iquad, iface, idim, idof;

  /*pointer on the values to fill */
  t8dg_element_quad_values_t *element_quad_trafo;       /*size: num_elem_quad */
  ...
  global_values = t8dg_local_values_get_eclass_global_values (local_values, eclass);

  num_elem_quad = t8dg_global_values_get_num_element_quad_vertices (global_values);
  num_faces = t8dg_global_values_get_num_faces (global_values);

  element_quad_trafo = t8dg_local_values_get_element_quad_trafo_weights (values, idata);
  ...
  for (iquad = 0; iquad < num_elem_quad; iquad++) {
    ...
    for (idim = 0; idim < t8dg_quadrature_get_dim (quadrature); idim++) {
      transformed_gradient_tangential_vector = t8dg_local_values_get_transformed_gradient_tangential_vector (values, idata, iquad, idim);
  ...
  for (iface = 0; iface < num_faces; iface++) {
    face_quad_trafo = t8dg_local_values_get_face_quad_trafo_weights (values, idata, iface);
    ...
    for (idof = 0; idof < num_face_dof; idof++) {
      ...
      image_normal_vector = t8dg_local_values_get_face_normal_vector (values, idata, iface, idof);
...
t8dg_local_values_t *
t8dg_local_values_new (t8_forest_t forest, t8dg_global_values_t * global_values[T8_ECLASS_COUNT], t8dg_coarse_geometry_t * coarse_geometry)
{
  ...
  for (eclass = 0; eclass < (int) T8_ECLASS_COUNT; eclass++) {
    if (global_values[eclass] != NULL) {
      local_values->max_num_faces = SC_MAX (t8dg_global_values_get_num_faces (global_values[eclass]), local_values->max_num_faces);
      local_values->max_num_elem_values =
        SC_MAX (t8dg_global_values_get_num_elem_quad (global_values[eclass]), local_values->max_num_elem_values);
      local_values->max_num_face_values =
        SC_MAX (t8dg_global_values_get_max_num_facevalues (global_values[eclass]), local_values->max_num_face_values);

      /* Only elements of the same dimension ? */
      local_values->dim = t8dg_global_values_get_dim (global_values[eclass]);
    }
    else {
      t8dg_debugf ("global_values has no entry for eclass = %i\n", eclass);
    }
  }
  local_values->global_values = global_values;

  /*for each element an array of double values */
  local_values->element_trafo_quad_weight = t8dg_quad_values_new_local (forest, global_values); /*? */

  for (idim = 0; idim < DIM3; idim++) {
    local_values->element_transformed_gradient_tangential_vectors[idim] = t8dg_quad_values_new_local (forest, global_values);

  }

  for (iface = 0; iface < local_values->max_num_faces; iface++) {
    local_values->face_trafo_quad_weight[iface] = t8dg_quad_values_new (forest, global_values);

    for (idim = 0; idim < DIM3; idim++) {
      local_values->face_normal_vectors[iface][idim] = t8dg_dof_values_new (forest, global_values);
    }
  }
...
t8dg_local_values_destroy (t8dg_local_values_t ** pvalues)
{
  ...
  for (idim = 0; idim < DIM3; idim++) {
    t8dg_quad_values_destroy (&values->element_transformed_gradient_tangential_vectors[idim]);
  }
  for (iface = 0; iface < values->max_num_faces; iface++) {
    t8dg_quad_values_destroy (&values->face_trafo_quad_weight[iface]);
    for (idim = 0; idim < DIM3; idim++) {
      t8dg_dof_values_destroy (&values->face_normal_vectors[iface][idim]);
    }
  }
  T8DG_FREE (values);
  *pvalues = NULL;
                                                   ...
                                                   t8dg_face_quad_values_t * src_face_quad, t8dg_face_quad_values_t * dest_face_quad)
{
  ...
  face_trafo_quad_weights = t8dg_quad_values_new_face_quad_values_view (local_values->face_trafo_quad_weight[iface], itree, ielement);
...
t8dg_local_values_partition (t8dg_local_values_t * local_values_old, t8dg_local_values_t * local_values_partition)
{
  ...
  for (idim = 0; idim < DIM3; idim++) {
    t8dg_quad_values_partition (local_values_old->element_transformed_gradient_tangential_vectors[idim],
                                local_values_partition->element_transformed_gradient_tangential_vectors[idim]);
  ...
  for (iface = 0; iface < local_values_old->max_num_faces; iface++) {
    ...
    for (idim = 0; idim < DIM3; idim++) {
      t8dg_dof_values_partition (local_values_old->face_normal_vectors[iface][idim],
                                 local_values_partition->face_normal_vectors[iface][idim]);
...
t8dg_local_values_get_global_values_from_itree (t8dg_local_values_t * local_values, t8_locidx_t itree)
{
  t8_eclass_t         eclass;
  eclass = t8_forest_get_eclass (local_values->forest, itree);
  return local_values->global_values[eclass];
}
...
void
t8dg_local_values_transform_face_child_dof_to_parent_dof (const t8dg_global_values_t * global_values,
                                                          ...
                                                          t8_locidx_t idata_child[MAX_SUBFACES], t8_locidx_t idata_parent, int iface_parent)
{
  sc_array_t         *summand;
  ...
  for (ichild = 0; ichild < num_face_children; ichild++) {
    t8dg_local_values_apply_face_mass_matrix (global_values, local_values, idata_child[ichild], iface_parent,
  ...
  t8dg_local_values_apply_face_inverse_mass_matrix (global_values, local_values, idata_parent, iface_parent, parent_face_dof,
                                                    parent_face_dof);
...
void
t8dg_local_values_transform_child_dof_to_parent_dof (const t8dg_global_values_t * global_values,
                                                     ...
                                                     t8_locidx_t idata_first_child, t8_locidx_t idata_parent)
{
  ...
  for (ichild = 0; ichild < num_children; ichild++) {
    t8dg_local_values_apply_element_mass_matrix (global_values, local_values_old, idata_first_child + ichild,
  ...
  t8dg_local_values_apply_element_inverse_mass_matrix (global_values, local_values_new, idata_parent, parent_dof, parent_dof);
  sc_array_destroy (mass_times_child_dof);
  sc_array_destroy (summand);
}
