...
static double *
t8dg_element_quad_get_transformed_gradient_tangential_vector(t8dg_advect_problem_linear_1D_t *problem, t8_locidx_t ielement, t8dg_quad_idx_t iquad)
{
...
t8dg_advect_element_get_face_mortar();
#endif
...
static void t8dg_element_set_precalculated_values_1D_linear(t8dg_advect_problem_linear_1D_t *problem,t8_locidx_t ielement,
...
static void t8dg_advect_evolution(sc_array_t *dudt_array, const sc_array_t *u_array, double t, const void *application_data){
				   ...
				   sc_MPI_Comm comm)
{
  ...
  problem->element_trafo_quad_weight =
    sc_array_new_count (sizeof (double) * t8dg_LGL_quadrature_get_num_element_vertices(problem->quadrature),
			num_elements);

  for(iface=0; iface < t8dg_LGL_quadrature_get_num_faces(problem->quadrature); iface++){
      problem->face_trafo_quad_weight[iface] =
        sc_array_new_count (sizeof (double) * t8dg_LGL_quadrature_get_num_face_vertices(problem->quadrature, iface),
			    num_elements);
...
#if 0
void t8dg_apply_mass_matrix(sc_array_t *dest, const sc_array_t *src, const void *application_data){
...
void t8dg_apply_stiffness_matrix(sc_array_t *dest, const sc_array_t *src, const void *application_data){
  T8_ASSERT(application_data != NULL);
  const sc_array_t *element_dof_values;
  sc_array_t *element_dof_values_new;
  int idata;
